...
\begin{latexcode}
\input{config/preambule}   % classe, géométrie, paquets
\input{config/polices}     % fontspec (latin, CJK, arabe, éthiopien)
\input{config/langues}     % polyglossia (français + langues secondaires)
\input{config/biblio}      % biblatex + biber (APA 7)
\input{config/style-fti}   % couleurs de la charte, en-têtes, titres
